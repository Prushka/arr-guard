...
func pendingQueueItem(id, episode int, origin, reason string) QueueRecord {
	return QueueRecord{ID: id, EpisodeID: episode, SeriesID: 3, MovieID: 3, DownloadID: origin, Status: "completed", TrackedDownloadState: "importPending", StatusMessages: []QueueStatusMessage{{Messages: []string{reason}}}}
}

func newPartialQueueFixture(t *testing.T) *safetyFixture {
	t.Helper()
	f := newSafetyFixture(t, "sonarr")
	f.episodes = []Episode{{ID: 10, SeriesID: 3, EpisodeFileID: 17}, {ID: 12, SeriesID: 3, EpisodeFileID: 17}, {ID: 14, SeriesID: 3}, {ID: 16, SeriesID: 3}}
...
func TestQueueExistingMediaOnlyCleansUpWithoutSearchOrRetries(t *testing.T) {
	for _, kind := range []string{"sonarr", "radarr"} {
		t.Run(kind, func(t *testing.T) {
			...
			f.queue = []QueueRecord{pendingQueueItem(7, 10, "pack", "Not an upgrade for existing episode")}
			for range 3 {
...
func TestQueueMissingMovieSearchAndDryRun(t *testing.T) {
	for _, dry := range []bool{false, true} {
		...
		f.queue = []QueueRecord{pendingQueueItem(7, 0, "pack", "Unable to determine if file is a sample")}
		if err := f.service.recoverBlockedQueue(t.Context(), f.client); err != nil {
			t.Fatal(err)
		}
		if dry {
	...
	for _, kind := range []string{"sonarr", "radarr"} {
		for _, scenario := range []string{"existing", "missing", "dryRun"} {
			t.Run(kind+"/"+scenario, func(t *testing.T) {
				...
				f.queue = []QueueRecord{pendingQueueItem(7, 10, "pack", "No files found are eligible for import")}
				f.history = []HistoryRecord{
...
func TestQueuePartialImportedRowsAndHistoryCompleteTheMapping(t *testing.T) {
	...
	for i, episode := range f.episodes {
		f.history = append(f.history, HistoryRecord{ID: i + 1, SeriesID: 3, EpisodeID: episode.ID, DownloadID: "pack", EventType: "grabbed"})
	}
	if err := f.service.recoverBlockedQueue(t.Context(), f.client); err != nil {
		t.Fatal(err)
	}
...
func TestQueueRecoveryRefusesUnreliablePreflight(t *testing.T) {
	for _, failure := range []string{"active", "ordinaryPending", "missingImportedFile", "crossSubject", "crossHistory", "missingHistory", "historyFailure", "missingEpisode", "missingMapping", "reassignedID", "changedQueue", "canceled", "retryLimit", "journalFailure"} {
		t.Run(failure, func(t *testing.T) {
			...
			case "crossHistory":
				f.history = []HistoryRecord{{ID: 1, SeriesID: 4, EpisodeID: 14, DownloadID: "pack", EventType: "grabbed"}}
			case "missingHistory":
...
func TestQueueHistoryResolvesUnparsedSubjectAndEpisodes(t *testing.T) {
	for _, kind := range []string{"sonarr", "radarr"} {
		...
		f.queue = []QueueRecord{q}
		f.history = []HistoryRecord{{ID: 1, SeriesID: 3, MovieID: 3, EpisodeID: 10, DownloadID: "pack", EventType: "grabbed"}}
		if err := f.service.recoverBlockedQueue(t.Context(), f.client); err != nil {
			t.Fatal(err)
		}
		if len(f.commands) != 1 {
...
func TestQueueDuplicateDownloadsDoNotRepeatSearch(t *testing.T) {
	f := newPartialQueueFixture(t)
	copyOfQueue := append([]QueueRecord(nil), f.queue...)
...
func TestQueueRecoveryHonorsOptInInScansAndServe(t *testing.T) {
	for _, scan := range []bool{false, true} {
		for _, enabled := range []bool{false, true} {
			...
			f.queue = []QueueRecord{pendingQueueItem(7, 0, "pack", "Not an upgrade for existing movie")}
			if scan {
