...
func fixtureDiagnosticProber(t *testing.T, output, diagnostic string, exitCode int) Prober {
	t.Helper()
	dir := t.TempDir()
	...
	return Prober{Path: probe, Timeout: 5 * time.Second}
}
...
func TestRecoveredVideoDiagnosticsApplySubtitlePolicy(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			if !slices.Equal(v.ProbeWarnings, []string{recoveredMPEG2Warning}) || v.fileInfo == nil || !v.sidecarsChecked {
...
func TestProbeDiagnosticFormattingPreservesCauseAndBounds(t *testing.T) {
	err := withProbeDiagnostics(context.DeadlineExceeded, recoveredVideoError+"\n"+recoveredVideoError+"\n[codec @ 0xabc] [error] broken\x1b\rmessage")
