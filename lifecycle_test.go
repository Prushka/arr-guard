...
func TestRunDryModesAndBindFailureNeverMutate(t *testing.T) {
	for _, mode := range []string{"subtitles", "unmatched", "serve"} {
		t.Run(mode, func(t *testing.T) {
			...
			t.Setenv("RADARR_URL", f.client.config.URL)
			t.Setenv("RADARR_API_KEY", "test")
			t.Setenv("RADARR_API_VERSION", "v3")
			t.Setenv("DRY_RUN", "true")
			t.Setenv("MODE", mode)
			mappings, err := json.Marshal([]PathMapping{{From: filepath.Dir(f.file.Path), To: filepath.Dir(f.file.Path)}})
			if err != nil {
				t.Fatal(err)
			}
...
func TestAuditContinuesAcrossInstances(t *testing.T) {
	...
	sonarr.service.probeFn = func(_ context.Context, path string) (Validation, error) {
		if path == sonarr.file.Path {
			return Validation{}, errors.New("injected probe failure")
		}
		v := radarr.validation
		v.Valid = true
		return v, nil
...
func TestUnmatchedFollowsConfiguredRootAliasWithoutFalseOrphans(t *testing.T) {
	...
	f.service.config.PathMappings = []PathMapping{{From: root, To: alias}}
...
func TestNewServiceEnforcesReadOnlyClientAndExclusiveState(t *testing.T) {
	...
	for _, mode := range []string{"serve", "subtitles", "unmatched"} {
		cfg := Config{Mode: mode, DryRun: true, Workers: 1, StatePath: filepath.Join(t.TempDir(), "state.json"), Radarr: &ArrConfig{Kind: "radarr", Name: "radarr", URL: "http://example.invalid", APIVersion: "v3"}}
		s, err := NewService(cfg, log)
		if err != nil {
			t.Fatal(err)
		}
		if !s.arr["radarr"].readOnly {
			t.Fatal("service did not set client read-only")
		}
		_ = s.state.Close()
	}
	cfg := Config{Mode: "serve", Workers: 1, StatePath: filepath.Join(t.TempDir(), "state.json"), Radarr: &ArrConfig{Kind: "radarr", Name: "radarr", URL: "http://example.invalid", APIVersion: "v3"}}
	s, err := NewService(cfg, log)
	if err != nil {
		t.Fatal(err)
	}
