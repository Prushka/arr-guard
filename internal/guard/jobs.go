...
func storedWebhook(kind string, payload arr.WebhookPayload) (string, StoredWebhook, error) {
	...
	minimal := arr.WebhookPayload{EventType: "Download"}
	...
	for _, id := range arr.CanonicalIDs(ids) {
		if kind == "sonarr" {
			minimal.EpisodeFiles = append(minimal.EpisodeFiles, arr.WebhookFile{ID: id})
		} else {
			minimal.MovieFiles = append(minimal.MovieFiles, arr.WebhookFile{ID: id})
		}
	}
	if payload.Series != nil {
		minimal.Series = &arr.Series{ID: payload.Series.ID}
	}
	if payload.Movie != nil {
		minimal.Movie = &arr.Movie{ID: payload.Movie.ID}
	}
