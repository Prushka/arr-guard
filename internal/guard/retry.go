...
func canRetryProcessing(err error) bool {
	...
	var changed *probe.SnapshotChangedError
...
func (s *Service) auditFileWithRetries(ctx context.Context, client *arr.Client, file arr.MediaFile) error {
	...
	err := retryScanReads(ctx, func() error {
		...
		if client.Kind() == "sonarr" {
			...
			var movie arr.Movie
			movie, readErr = client.GetMovie(ctx, fresh.MovieID)
