...
func retryKeys(kind string, file arr.MediaFile, ids []int) []string {
	if kind == "radarr" {
...
func (s *StateStore) Bind(clients map[string]*arr.Client) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	for kind, c := range clients {
		identity := fmt.Sprintf("%x", sha256.Sum256([]byte(c.BaseURL())))
	...
	return s.updateLocked(func(next *State) {
		for kind, c := range clients {
			next.Instances[kind] = fmt.Sprintf("%x", sha256.Sum256([]byte(c.BaseURL())))
		}
	})
}

func sameMedia(a, b arr.MediaFile, kind string) bool {
	return a.ID == b.ID && a.SubjectID(kind) == b.SubjectID(kind) && pathutil.Normalize(a.Path) == pathutil.Normalize(b.Path) && a.Size == b.Size && a.RelativePath == b.RelativePath && ((a.DateAdded == nil && b.DateAdded == nil) || (a.DateAdded != nil && b.DateAdded != nil && a.DateAdded.Equal(*b.DateAdded)))
}
...
func (s *Service) prepareOrigin(ctx context.Context, c *arr.Client, file arr.MediaFile, downloadID string) (originAction, error) {
	if downloadID == "" {
	...
	if queueID > 0 {
		if c.Kind() == "sonarr" {
			episodes, err := c.Episodes(ctx, file.ParentID)
			if err != nil {
				return originAction{}, err
			}
...
func (s *Service) applyOrigin(ctx context.Context, c *arr.Client, action originAction, reason string) error {
	if action.queueID > 0 {
		queue, err := c.Queue(ctx)
		if err != nil {
			return err
...
func openServiceState(cfg config.Config) (*StateStore, error) {
	if cfg.Mode == "unmatched" {
		return &StateStore{state: State{Attempts: map[string]int{}}}, nil
	}
	cfg.StatePath = pathutil.ResolveExisting(cfg.StatePath)
	if cfg.DryRun {
...
func (s *Service) remainingSearchTargets(ctx context.Context, c *arr.Client, subjectID int, ids []int) ([]int, bool, error) {
	if c.Kind() == "sonarr" {
		episodes, err := c.Episodes(ctx, subjectID)
		if err != nil {
		...
		movie, err := c.GetMovie(ctx, subjectID)
		if err != nil {
			return nil, false, err
		}
		...
		files, err := c.MovieFiles(ctx, subjectID)
		if err != nil {
			return nil, false, err
		}
...
func (s *Service) searchRemaining(ctx context.Context, c *arr.Client, key string, subjectID int, ids []int) error {
