...
func TestUnrelatedDirectoryActivityDoesNotBlockValidation(t *testing.T) {
	for _, kind := range []string{"sonarr", "radarr"} {
		for _, valid := range []bool{false, true} {
			...
			if err := os.Chtimes(dir, time.Now(), f.validation.DirectoryInfo.ModTime().Add(time.Second)); err != nil {
				t.Fatal(err)
			}
...
func TestMatchingSubtitleChangesRemainProtected(t *testing.T) {
	for _, change := range []string{"add", "remove", "rename", "contents", "missingSnapshot"} {
		t.Run(change, func(t *testing.T) {
			...
			var err error
			f.validation.Sidecars, err = probe.DiscoverExternalSubtitles(f.file.Path)
			if err != nil {
				t.Fatal(err)
			}
			switch change {
			...
			if err := os.Chtimes(dir, time.Now(), f.validation.DirectoryInfo.ModTime()); err != nil {
				t.Fatal(err)
			}
			if err := f.apply(t.Context()); err == nil {
	...
	for _, kind := range []string{"sonarr", "radarr"} {
		for _, valid := range []bool{true, false} {
			...
			if valid {
				...
				f.history = []arr.HistoryRecord{{ID: 2, SeriesID: 3, MovieID: 3, DownloadID: "origin", EventType: "downloadFolderImported", Data: map[string]string{"fileId": "17"}}, {ID: 1, SeriesID: 3, MovieID: 3, DownloadID: "origin", EventType: "grabbed"}}
...
func TestPartialReplacementsOnlySearchRemainingEpisodes(t *testing.T) {
	for _, allReplaced := range []bool{false, true} {
		...
		f.replacementEpisodes = []arr.Episode{{ID: 10, SeriesID: 3, EpisodeFileID: 99}, {ID: 12, SeriesID: 3}}
		if allReplaced {
...
func TestMovieReplacementCompletesWithoutDuplicateSearch(t *testing.T) {
	f := newSafetyFixture(t, "radarr")
	f.replacementFiles = []arr.MediaFile{{ID: 99, MovieID: 3}}
	if err := f.apply(t.Context()); err != nil {
		t.Fatal(err)
	}
...
func TestQueueRemovalFiltersLaterReplacementsAndPreservesExistingMedia(t *testing.T) {
	f := newSafetyFixture(t, "sonarr")
	f.deleted = true
	f.queue = []arr.QueueRecord{{ID: 7, DownloadID: "origin", SeriesID: 3, EpisodeID: 10, Status: "completed", TrackedDownloadState: "importBlocked"}, {ID: 8, DownloadID: "origin", SeriesID: 3, EpisodeID: 12, Status: "completed", TrackedDownloadState: "importBlocked"}}
	f.before = func(r *http.Request) {
	...
	f = newSafetyFixture(t, "sonarr")
	f.queue = []arr.QueueRecord{{ID: 7, DownloadID: "origin", SeriesID: 3, EpisodeID: 10, Status: "completed", TrackedDownloadState: "importBlocked"}}
	...
	for _, persistent := range []bool{false, true} {
		...
		f.service.probeFn = func(context.Context, string) (probe.Validation, error) {
			...
			if persistent || probes == 1 {
				return probe.Validation{}, errors.New("temporary inaccessible fixture")
			}
		...
		if persistent {
			...
			f.service.probeFn = func(context.Context, string) (probe.Validation, error) { return f.validation, nil }
...
func TestScanDoesNotRetryMutationFailureOrWriteDryRunJobs(t *testing.T) {
	...
	f.service.probeFn = func(context.Context, string) (probe.Validation, error) {
		return probe.Validation{}, errors.New("temporary probe failure")
	}
...
func TestDelayedEpisodeMappingResumesWithoutReview(t *testing.T) {
	...
	payload := arr.WebhookPayload{EventType: "Download", EpisodeFile: &arr.WebhookFile{ID: 17}}
	err := f.service.processWebhook(t.Context(), f.client, payload)
...
func TestRetryClassificationPreservesMutationBoundary(t *testing.T) {
	readFailure := &arr.HTTPError{Method: http.MethodGet, Status: http.StatusServiceUnavailable}
	for _, test := range []struct {
		...
		{"APIRateLimit", &arr.HTTPError{Method: http.MethodGet, Status: http.StatusTooManyRequests}, true},
		{"APICredentials", &arr.HTTPError{Method: http.MethodGet, Status: http.StatusUnauthorized}, false},
		{"PostUnavailable", &arr.HTTPError{Method: http.MethodPost, Status: http.StatusServiceUnavailable}, false},
...
func TestLegacyExhaustedJobsResumeEvaluationWithJournalProtection(t *testing.T) {
	for _, pending := range []bool{false, true} {
		f := newSafetyFixture(t, "radarr")
		key, stored, err := storedWebhook("radarr", arr.WebhookPayload{EventType: "Download", MovieFile: &arr.WebhookFile{ID: 17}})
		if err != nil {
			t.Fatal(err)
		}
		stored.Failures = 5
