...
func queueRecoveryGroup(kind string, item arr.QueueRecord, queue []arr.QueueRecord, historySubject int) (int, []queueRecoveryRow, error) {
...
func (s *Service) planQueueRecovery(ctx context.Context, c *arr.Client, item arr.QueueRecord, searched map[string]bool) (queueRecoveryPlan, error) {
	...
	plan.episodeIDs = arr.CanonicalIDs(ids)
...
func queueSearchTargets(kind string, subject int, origin string, ids []int, needed bool, queue []arr.QueueRecord, searched map[string]bool) ([]int, bool) {
	if !needed {
		return nil, false
	}
	...
	movieCovered := false
	if kind == "radarr" {
		movieCovered = searched[retryKeys(kind, arr.MediaFile{MovieID: subject}, nil)[0]]
	}
	for _, q := range queue {
	...
	var remaining []int
	for _, id := range ids {
		keys := retryKeys(kind, arr.MediaFile{ParentID: subject}, []int{id})
...
func (s *Service) recoverBlockedQueueItem(ctx context.Context, c *arr.Client, item arr.QueueRecord) error {
	return s.recoverQueueItem(ctx, c, item, map[string]bool{})
}

func (s *Service) recoverQueueItem(ctx context.Context, c *arr.Client, item arr.QueueRecord, searched map[string]bool) (resultErr error) {
