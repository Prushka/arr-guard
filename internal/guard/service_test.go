...
func TestWebhookPayloadFiles(t *testing.T) {
	sonarr := arr.WebhookPayload{EpisodeFile: &arr.WebhookFile{ID: 1}}
	if got := sonarr.Files("sonarr"); len(got) != 1 || got[0].ID != 1 {
		t.Fatalf("unexpected Sonarr files: %#v", got)
	}
	radarr := arr.WebhookPayload{MovieFiles: []arr.WebhookFile{{ID: 2}, {ID: 3}}}
...
func TestOldUnknownSubtitleGrace(t *testing.T) {
	now := time.Now()
	validation := probe.Validation{HasSubtitles: true, HasUnknownLanguage: true, Reason: "no English subtitle stream or sidecar"}
	...
	if noSubtitles := applyOldMediaGrace(probe.Validation{HasUnknownLanguage: true}, now.Year()-11, now); noSubtitles.Valid {
...
func TestAuditSkipsSilentMediaBeforeProbe(t *testing.T) {
	...
	service := &Service{
		...
		probeFn: func(context.Context, string) (probe.Validation, error) {
			probed = true
			return probe.Validation{}, nil
		},
	}
	file := arr.MediaFile{ID: 7, MovieID: 3, Year: time.Now().Year() - 51, Path: "Movie.mkv"}
...
func TestWebhookSkipsSilentMovieBeforeProbe(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method == http.MethodGet && r.URL.Path == "/api/v3/movie/3" {
			_ = json.NewEncoder(w).Encode(arr.Movie{ID: 3, Year: time.Now().Year() - 51})
			return
		}
	...
	payload := arr.WebhookPayload{
		...
		Movie:      &arr.Movie{ID: 3, Year: time.Now().Year() - 51},
		MovieFile:  &arr.WebhookFile{ID: 7},
	}
...
func TestWebhookUsesEpisodeReleaseYearForSilentMediaSkip(t *testing.T) {
	...
	payload := arr.WebhookPayload{
		EventType:   "Download",
		DownloadID:  "download-id",
		Series:      &arr.Series{ID: 3, Year: 1920},
		EpisodeFile: &arr.WebhookFile{ID: 7},
	}
...
func TestWebhookDoesNotSkipModernEpisodeOfOldSeries(t *testing.T) {
	...
	f.history = []arr.HistoryRecord{{ID: 1, SeriesID: 3, DownloadID: "download-id", EventType: "downloadFolderImported", Data: map[string]string{"fileId": "17"}}}
	...
	payload := arr.WebhookPayload{
		...
		DownloadID:  "download-id",
		Series:      &arr.Series{ID: 3, Year: 1920},
		EpisodeFile: &arr.WebhookFile{ID: 17},
	}
...
func TestHistoryDataIsCaseInsensitive(t *testing.T) {
	record := arr.HistoryRecord{Data: map[string]string{"FileId": "17"}}
...
func TestRetryKeyUsesStableSonarrEpisodeIDs(t *testing.T) {
	file := arr.MediaFile{ParentID: 3, RelativePath: "Show.mkv"}
...
func TestRetryKeyUsesStableRadarrMovieID(t *testing.T) {
	file := arr.MediaFile{MovieID: 3, RelativePath: "Movie.mkv"}
...
func TestWebhookHandlerAuthAndEnqueue(t *testing.T) {
	...
	service := &Service{
		config: config.Config{WebhookToken: "token"},
		arr:    map[string]*arr.Client{"sonarr": client},
		jobs:   make(chan webhookJob, 1),
	}
	payload, err := json.Marshal(arr.WebhookPayload{EventType: "Download", EpisodeFile: &arr.WebhookFile{ID: 7}})
	if err != nil {
		t.Fatal(err)
	}
...
func TestWebhookHandlerBasicAuth(t *testing.T) {
	client := testArrClient("sonarr", "http://arr.invalid")
	service := &Service{
		config: config.Config{WebhookUsername: "sonarr", WebhookPassword: "secret"},
		arr:    map[string]*arr.Client{"sonarr": client},
		jobs:   make(chan webhookJob, 1),
	}
...
func TestRecoverBlockedQueueGroupsDownloadAndLimitsSearches(t *testing.T) {
	for _, kind := range []string{"sonarr", "radarr"} {
		t.Run(kind, func(t *testing.T) {
			...
			f.queue = []arr.QueueRecord{{ID: 7, DownloadID: "blocked", MovieID: 3, SeriesID: 3, EpisodeID: 10, Status: "completed", TrackedDownloadState: "importBlocked"}}
			if kind == "sonarr" {
				second := f.queue[0]
				second.ID = 8
				second.EpisodeID = 12
...
func TestRecoverBlockedQueueDryRunDoesNotMutate(t *testing.T) {
	...
	f.service.config.DryRun = true
	f.deleted = true
	f.queue = []arr.QueueRecord{{ID: 7, DownloadID: "blocked", MovieID: 3, Status: "completed", TrackedDownloadState: "importBlocked"}}
	if err := f.service.recoverBlockedQueue(t.Context(), f.client); err != nil {
		t.Fatal(err)
	}
...
func TestQueueRecordNeedsImportRecovery(t *testing.T) {
	tests := []struct {
		name string
		item arr.QueueRecord
		want bool
	}{
		{name: "blocked state", item: arr.QueueRecord{Status: "completed", TrackedDownloadState: "importBlocked"}, want: true},
		{name: "blocked message", item: arr.QueueRecord{Status: "completed", StatusMessages: []arr.QueueStatusMessage{{Messages: []string{"Unable to Import Automatically"}}}}, want: true},
		{name: "still downloading", item: arr.QueueRecord{Status: "downloading", TrackedDownloadState: "importBlocked"}, want: false},
		{name: "ordinary completed", item: arr.QueueRecord{Status: "completed", TrackedDownloadState: "importPending"}, want: false},
	}
...
func TestMapPath(t *testing.T) {
	...
	service := &Service{
		config: config.Config{PathMappings: []config.PathMapping{{From: "/tv", To: "/media/tv"}}},
		log:    slog.Default(),
		state:  store,
	}
	...
	service.config.PathMappings = []config.PathMapping{{From: `C:\Media\TV`, To: `D:\Mounted\TV`}}
	if got := service.mapPath(`C:\Media\TV\Show\episode.mkv`); got != `D:\Mounted\TV\Show\episode.mkv` {
		t.Fatalf("Windows mapped path = %q", got)
	}
	service.config.PathMappings = []config.PathMapping{{From: "/tv", To: "/media/tv"}}
	...
	service.config.PathMappings = []config.PathMapping{
...
func TestScanRootsDeduplicatesNestedDestinations(t *testing.T) {
	...
	got := scanRoots([]config.PathMapping{
...
func TestScanUnmatchedWritesAllOrphansWithoutProbing(t *testing.T) {
	...
	service := &Service{
		config: config.Config{
			PathMappings:         []config.PathMapping{{From: "/media", To: root}},
			UnmatchedPath:        reportPath,
			Workers:              2,
			UnmatchedExcludeDirs: []string{filepath.Join(root, "Excluded")},
		},
		log: slog.Default(),
		arr: map[string]*arr.Client{"sonarr": testArrClient("sonarr", server.URL)},
	}
