...
func Load() (Config, error) {
	...
	if raw := strings.TrimSpace(os.Getenv("PATH_MAPPINGS_JSON")); raw != "" {
		...
		for i, mapping := range cfg.PathMappings {
			...
			for _, previous := range cfg.PathMappings[:i] {
				one, two := pathutil.ComparableArr(previous.From, mapping.From)
				if one == two {
	...
	if pathutil.Key(pathutil.ResolveExisting(cfg.StatePath)) == pathutil.Key(pathutil.ResolveExisting(cfg.UnmatchedPath)) {
	...
	for _, output := range []string{cfg.StatePath, cfg.UnmatchedPath} {
		...
		if pathutil.IsMedia(output) || pathutil.IsSubtitle(output) {
		...
		for _, mapping := range cfg.PathMappings {
			if pathutil.Within(pathutil.ResolveExisting(mapping.To), pathutil.ResolveExisting(output)) {
...
func loadArrConfig(prefix, name, kind string) (*Arr, error) {
