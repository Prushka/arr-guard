	...
	for _, test := range tests {
		t.Run(test.kind, func(t *testing.T) {
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				...
				if got.Name != test.want.Name || got.SeriesID != test.want.SeriesID || !slices.Equal(got.MovieIDs, test.want.MovieIDs) || !slices.Equal(got.EpisodeIDs, test.want.EpisodeIDs) {
...
func testArrClient(kind, baseURL string) *Client {
	return NewClient(config.Arr{Name: kind, Kind: kind, URL: baseURL, APIKey: "secret", APIVersion: "v3"}, slog.Default())
}
