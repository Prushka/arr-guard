...
func TestQueueAndAlreadyFailedOriginsStillUseGuardSearch(t *testing.T) {
	for _, kind := range []string{"sonarr", "radarr"} {
		for _, queued := range []bool{false, true} {
			...
			f.autoRedownload = true
			if queued {
				f.queue = []QueueRecord{{ID: 7, MovieID: 3, SeriesID: 3, EpisodeID: 10, DownloadID: "origin", Status: "completed", TrackedDownloadState: "imported"}}
			} else {
				f.history = append(f.history, HistoryRecord{ID: 3, MovieID: 3, SeriesID: 3, DownloadID: "origin", EventType: "downloadFailed"})
			}
			if err := f.apply(t.Context()); err != nil {
				t.Fatal(err)
			}
...
func TestAutomaticHistorySearchDryRunPreservesStateAndMedia(t *testing.T) {
	for _, kind := range []string{"sonarr", "radarr"} {
		f := historySearchFixture(t, kind, "Rss")
		f.autoRedownload = true
		f.service.config.DryRun, f.client.readOnly = true, true
