...
func TestLiveProbeDiagnostics(t *testing.T) {
	...
	service := &Service{config: cfg, log: log, arr: clients, state: &StateStore{state: State{Attempts: map[string]int{}}}, probe: Prober{Path: cfg.FFprobePath, Timeout: 2 * time.Minute}}
	...
	for _, value := range values {
		...
		t.Run(kind+"/"+idText, func(t *testing.T) {
			...
			if kind == "sonarr" {
				...
				var movie Movie
				err = client.do(t.Context(), http.MethodGet, client.apiPath("movie", strconv.Itoa(file.MovieID)), nil, nil, &movie)
...
func liveSonarrProbeFileID(t *testing.T, client *ArrClient, path string) int {
	t.Helper()
	if client == nil {
		t.Fatal("Sonarr is required for lookup by reported path")
	}
	var series []Series
	if err := client.do(t.Context(), http.MethodGet, client.apiPath("series"), nil, nil, &series); err != nil {
	...
	var files []MediaFile
	if err := client.do(t.Context(), http.MethodGet, client.apiPath("episodefile"), url.Values{"seriesId": {strconv.Itoa(owner.ID)}}, nil, &files); err != nil {
