...
func TestDryRunSerializesPreflightsWithoutMutations(t *testing.T) {
	...
	f.client.client.Transport = safetyRoundTripFunc(func(r *http.Request) (*http.Response, error) {
		n := active.Add(1)
		defer active.Add(-1)
...
func TestConcurrentWebhooksSerializeOriginHistoryReads(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Path == "/api/v3/episode" {
			...
			for id := 17; id < 25; id++ {
				episodes = append(episodes, Episode{ID: id + 100, SeriesID: 3, EpisodeFileID: id, AirDate: "2025-01-01"})
			}
	...
	for id := 17; id < 25; id++ {
		wg.Go(func() {
			payload := WebhookPayload{EventType: "Download", DownloadID: "origin", Series: &Series{ID: 3}, EpisodeFile: &WebhookFile{ID: id}}
...
func newSafetyFixture(t *testing.T, kind string) *safetyFixture {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case r.Method == http.MethodGet && r.URL.Path == "/api/v3/series":
			respond([]Series{{ID: 3, Year: f.file.Year}})
		...
		case r.Method == http.MethodGet && r.URL.Path == "/api/v3/episode":
			episodes := append([]Episode(nil), f.episodes...)
			if f.deleted {
	...
	f.client = NewArrClient(ArrConfig{Kind: kind, Name: kind, URL: server.URL, APIKey: "test", APIVersion: "v3"}, log)
	f.service = &Service{config: Config{MaxAttempts: 3, Workers: 2}, log: log, state: store, arr: map[string]*ArrClient{kind: f.client}, probeFn: func(context.Context, string) (Validation, error) { return f.validation, nil }}
	return f
}
...
func TestMutationFailuresRemainJournaledWithoutReplay(t *testing.T) {
	for _, phase := range []string{"DELETE /api/v3/moviefile/17", "DELETE /api/v3/queue/7", "POST /api/v3/command"} {
		t.Run(phase, func(t *testing.T) {
			...
			f.history = []HistoryRecord{{ID: 2, MovieID: 3, DownloadID: "origin", EventType: "downloadFolderImported", Data: map[string]string{"fileId": "17"}}}
			f.queue = []QueueRecord{{ID: 7, MovieID: 3, DownloadID: "origin", Status: "completed", TrackedDownloadState: "imported"}}
			f.fail = phase
			if err := f.apply(t.Context()); err == nil {
...
func TestPreflightFailuresNeverDelete(t *testing.T) {
	for _, failure := range []string{"GET /api/v3/moviefile/17", "GET /api/v3/history/movie", "GET /api/v3/queue", "GET /api/v3/history", "GET /api/v3/config/downloadclient"} {
		t.Run(failure, func(t *testing.T) {
			f := newSafetyFixture(t, "radarr")
			f.history = []HistoryRecord{{ID: 2, MovieID: 3, DownloadID: "origin", EventType: "downloadFolderImported", Data: map[string]string{"fileId": "17"}}, {ID: 1, MovieID: 3, DownloadID: "origin", EventType: "grabbed"}}
			f.fail = failure
			if err := f.apply(t.Context()); err == nil {
	...
	for _, change := range []string{"path", "size", "parent", "mapping", "disk", "sidecar"} {
		t.Run(change, func(t *testing.T) {
			...
			case "parent":
				f.file.ParentID++
			case "mapping":
				f.episodes = []Episode{{ID: 10, SeriesID: 3, EpisodeFileID: 999}}
			case "disk":
...
func TestConcurrentDuplicateWebhooksAndAuditOnlyDeleteOnce(t *testing.T) {
	f := newSafetyFixture(t, "sonarr")
	payload := WebhookPayload{EventType: "Download", EpisodeFile: &WebhookFile{ID: 17}, Episodes: []Episode{{ID: 999}}}
	var wg sync.WaitGroup
	for i := 0; i < 20; i++ {
...
func TestStateLockAndServerBinding(t *testing.T) {
	...
	if err := a.Bind(map[string]*ArrClient{"radarr": testArrClient("radarr", "http://first.invalid")}); err != nil {
		t.Fatal(err)
	}
	if err := a.Bind(map[string]*ArrClient{"radarr": testArrClient("radarr", "http://second.invalid")}); err == nil {
...
func TestQueueRecoveryRefusesSharedActiveAndUncertainMutations(t *testing.T) {
	for _, scenario := range []string{"activeSibling", "retryLimit", "uncertainDelete"} {
		t.Run(scenario, func(t *testing.T) {
			f := newSafetyFixture(t, "sonarr")
			f.deleted = true
			f.queue = []QueueRecord{{ID: 7, DownloadID: "pack", SeriesID: 3, EpisodeID: 10, Status: "completed", TrackedDownloadState: "importBlocked"}}
			switch scenario {
			case "activeSibling":
				q := f.queue[0]
				q.ID = 8
...
func TestPathMappingPreservesUNCAndWindowsCase(t *testing.T) {
	s := &Service{config: Config{PathMappings: []PathMapping{{From: `C:\Media`, To: `\\server\share\media`}}}}
	if got := s.mapPath(`c:\media\Show\file.mkv`); got != `\\server\share\media\Show\file.mkv` {
		t.Fatalf("UNC/case mapping failed: %q", got)
	}
	s.config.PathMappings = []PathMapping{{From: "/media", To: "/mnt/media"}}
...
func TestWebhookOriginMustBeConfirmedAndImportedPackIsSafeToFail(t *testing.T) {
	f := newSafetyFixture(t, "sonarr")
	f.history = []HistoryRecord{{ID: 2, SeriesID: 3, DownloadID: "origin", EventType: "downloadFolderImported", Data: map[string]string{"fileId": "17"}}}
	f.queue = []QueueRecord{{ID: 7, SeriesID: 3, EpisodeID: 10, DownloadID: "origin", Status: "completed", TrackedDownloadState: "imported"}, {ID: 8, SeriesID: 3, EpisodeID: 12, DownloadID: "origin", Status: "completed", TrackedDownloadState: "imported"}}
	payload := WebhookPayload{EventType: "Download", EpisodeFile: &WebhookFile{ID: 17}, DownloadID: "wrong"}
