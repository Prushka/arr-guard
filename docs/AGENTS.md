...
Keep this file and [README.md](README.md) consistent with code changes. This
repository is a Go 1.26 application organized into internal packages, using the
standard library with PowerShell launch/lint scripts and Docker deployment files.
No generated API client is used. Run commands from the repository root.

Full documentation lives in `docs/`. The root `AGENTS.md` only forwards contributors
here. Save logs and coverage output in ignored `logs/`; build artifacts go in
ignored `bin/`. Do not move or print `.env` or commit private runtime artifacts.
...
- `*_test.go`: tests live beside their owning package; workflow/live integration
  tests live in `internal/guard`. Live checks remain explicitly opt-in.

Package dependencies flow from guard to config/arr/probe/pathutil. Path utilities
are independent, configuration uses path utilities, and probing does not import
Arr or guard. Avoid compatibility aliases or importing guard from leaf packages.
State JSON, runtime defaults, read-only transports, request deadlines, and snapshot
checks must remain compatible when changing package boundaries.
...
scope checks are unchanged. The probe package exposes a snapshot-change error;
guard classifies it for fresh reads only, with uncertain-operation errors still
taking precedence. Filesystem snapshot fields remain excluded from validation JSON.
Arr transport injection preserves read-only enforcement, redirect refusal, and the
30-second request deadline. All 127 existing test functions were retained; split
Unicode/date tests and a snapshot-serialization regression also pass.

Native tests, race detection, vet, lint, command help, and Linux/amd64 application
and test compilation passed. A temporary launcher fixture verified daily log
append, both output streams, environment overrides, and failed exit propagation
without reading the real `.env` or making requests. Live GET/dry-run verification
checked 11,350 accessible files with no size mismatches, both webhook/snapshot
