...
func TestDurableWebhookAdmissionSurvivesRestartAndRetries(t *testing.T) {
	...
	payload := WebhookPayload{EventType: "Download", EpisodeFiles: []WebhookFile{{ID: 17}, {ID: 17}}, DownloadID: "do-not-trust", Episodes: []Episode{{ID: 999}}}
...
func TestWorkersStopPreservesAcceptedJobsAndRejectsEnqueue(t *testing.T) {
	...
	f.service.probeFn = func(ctx context.Context, _ string) (Validation, error) {
		close(entered)
		<-ctx.Done()
...
func TestCanceledWorkerPreservesJobBeforeStopFlagIsSet(t *testing.T) {
	f := newSafetyFixture(t, "radarr")
	if err := f.service.Enqueue(f.client, WebhookPayload{EventType: "Download", MovieFile: &WebhookFile{ID: 17}}); err != nil {
		t.Fatal(err)
	}
	f.service.jobs = make(chan webhookJob, 1)
	f.service.dispatchStoredJobs()
