...
func TestLiveTargetedPreflight(t *testing.T) {
	...
	c := NewArrClient(*cfg.Sonarr, log)
	c.readOnly = true
	c.client.Transport = rt
	s := &Service{config: cfg, log: log, state: &StateStore{state: State{Attempts: map[string]int{}}}, arr: map[string]*ArrClient{"sonarr": c}, probe: Prober{Path: cfg.FFprobePath, Timeout: 2 * time.Minute}}
	check := func(id int) {
		...
		t.Logf("file_id=%d valid=%t preflight=%s preflight_elapsed=%s HTTP_timeout=%s mutations=0", id, v.Valid, category, time.Since(started).Round(time.Millisecond), c.client.Timeout)
...
func TestLiveReadOnly(t *testing.T) {
	...
	log := slog.New(slog.NewTextHandler(io.Discard, nil))
	allClients := map[string]*ArrClient{}
	for _, ac := range []*ArrConfig{cfg.Sonarr, cfg.Radarr} {
		...
		t.Run(ac.Kind, func(t *testing.T) {
			...
			client.client = &http.Client{Transport: rt, Timeout: 30 * time.Second, CheckRedirect: func(*http.Request, []*http.Request) error { return http.ErrUseLastResponse }}
			...
			service := &Service{config: cfg, log: log, state: &StateStore{state: State{Attempts: map[string]int{}}}, arr: map[string]*ArrClient{ac.Kind: client}, probe: Prober{Path: cfg.FFprobePath, Timeout: 2 * time.Minute}}
			...
			if os.Getenv("ARR_LIVE_WEBHOOK_CHECK") == "1" {
				for _, file := range files {
					...
					if err != nil {
						...
						v.dirInfo, err = os.Stat(t.TempDir())
						if err != nil {
...
func liveDiagnoseQueuedOrigins(t *testing.T, ctx context.Context, s *Service, c *ArrClient, files []MediaFile, queue []QueueRecord) {
	t.Helper()
	byID, bySubject := map[int]MediaFile{}, map[int]MediaFile{}
	for _, file := range files {
		byID[file.ID] = file
